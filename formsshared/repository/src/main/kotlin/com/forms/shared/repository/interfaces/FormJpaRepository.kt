...
interface FormJpaRepository : JpaRepository<Form, Long> {
    @Query("select new com.forms.shared.repository.dto.FormInfoOutputDTO(f.id, f.url, f.title, f.fillTime) from Form f " +
            "inner join f.user u " +
            "left join Response r on (r.user.id = :userId and f.id = r.form.id) " +
            "where u.id <> :userId " +
            "and r.id is null " +
            "order by u.ranking asc")
    fun findAvailableFormToAnsewer(@Param("userId") userId: Long, pageable: Pageable): Page<FormInfoOutputDTO>
}